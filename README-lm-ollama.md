...
2. 下載建議模型：`ollama pull llama3.2`（平衡）或 `ollama pull qwen2.5`（中文理解佳）。
   > ⚠️ 建議使用**非推理模型**。Qwen3.5 等推理模型會輸出 `<think>` 標籤，導致 JSON 解析失敗。
3. 啟動 Ollama 並開啟 CORS 設定：
...
4. 下載 `index-lm-ollama.html`，在瀏覽器中打開，右上角 Provider 選 **Ollama**，確認燈號為綠色。

---

### 方案 B：使用 LM Studio

1. 安裝 [LM Studio](https://lmstudio.ai/)，下載任意非推理模型（建議 `Qwen2.5` 系列或 `Llama 3.2`）。
   > ⚠️ 同樣建議使用**非推理模型**，原因同上。
2. 打開 LM Studio，進入 **Developer** 頁籤，點擊 **Start Server**（預設 port 1234）。
3. 下載 `index-lm-ollama.html`，在瀏覽器中打開，右上角 Provider 選 **LM Studio**，確認燈號為綠色。
